...
namespace banggame {
    ...
    struct request_miki : selection_picker {
        request_miki(card_ptr origin_card, player_ptr target, shared_request_draw &&req_draw)
    ...
    void equip_miki::on_enable(card_ptr target_card, player_ptr target) {
        target->m_game->add_listener<event_type::init_request_draw>({target_card, -1}, [target, target_card](player_ptr origin, shared_request_draw req_draw) {
            if (origin == target) {
                req_draw->handlers.push_back(target_card);
            }
        });

        target->m_game->add_listener<event_type::on_draw_from_deck>(target_card, [target, target_card](player_ptr origin, card_ptr origin_card, shared_request_draw req_draw) {
            if (origin == target && origin_card == target_card) {
                target->m_game->queue_request<request_miki>(target_card, target, std::move(req_draw));
            }
        });
    }
